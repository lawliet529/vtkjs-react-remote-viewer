...
class MPRViewer(vtk_protocols.vtkWebProtocol):
    ...
    def initCenterlineAxialView(self) -> None:
        ...
        linesAxial.AddInputConnection(greenLineAxial.GetOutputPort())
        linesAxial.AddInputConnection(blueLineAxial.GetOutputPort())
        linesAxial.Update()
    ...
    def initCenterlineCoronalView(self) -> None:
        ...
        linesCoronal.AddInputConnection(greenLineCoronal.GetOutputPort())
        linesCoronal.AddInputConnection(redLineCoronal.GetOutputPort())
        linesCoronal.Update()
    ...
    def initCenterlineSagittalView(self) -> None:
        ...
        linesSagittal.AddInputConnection(blueLineSagittal.GetOutputPort())
        linesSagittal.AddInputConnection(redLineSagittal.GetOutputPort())
    ...
    def initWidgetsAxialView(self) -> None:
        ...
        self.sphereWidgetAxial.ScaleOff()
        self.sphereWidgetAxial.SetRepresentation(0)
        self.sphereWidgetAxial.GetSphereProperty().SetColor(
            self.colors.GetColor3d("Green")
        )
        self.sphereWidgetAxial.GetSphereProperty().SetSpecular(1)
        self.sphereWidgetAxial.GetSphereProperty().SetDiffuse(0)
        ...
        self.sphereWidgetInteractionRotateGreenLineAxial.ScaleOff()
        self.sphereWidgetInteractionRotateGreenLineAxial.SetRepresentationToSurface()
        self.sphereWidgetInteractionRotateGreenLineAxial.GetSphereProperty().SetColor(
            self.colors.GetColor3d("Green")
        )
        self.sphereWidgetInteractionRotateGreenLineAxial.GetSelectedSphereProperty().SetOpacity(
            0
        )
        self.sphereWidgetInteractionRotateGreenLineAxial.SetCurrentRenderer(
            self.rendererAxial
        )
    ...
    def initWidgetsCoronalView(self) -> None:
        ...
        self.sphereWidgetCoronal.ScaleOff()
        self.sphereWidgetCoronal.SetRepresentationToOff()
        self.sphereWidgetCoronal.GetSphereProperty().SetColor(
            self.colors.GetColor3d("Green")
        )
    ...
    def initWidgetsSagittalView(self) -> None:
        ...
        self.sphereWidgetSagittal.ScaleOff()
        self.sphereWidgetSagittal.SetRepresentationToOff()
        self.sphereWidgetSagittal.GetSphereProperty().SetColor(
            self.colors.GetColor3d("Blue")
        )
    ...
    def show3DMPR(self) -> None:
        self.renderWindowAxial = self.getApplication().GetObjectIdMap().GetActiveObject("AXIAL_VIEW")
        self.renderWindowCoronal = self.getApplication().GetObjectIdMap().GetActiveObject("CORONAL_VIEW")
        self.renderWindowSagittal = self.getApplication().GetObjectIdMap().GetActiveObject("SAGITTAL_VIEW")

        self.renderWindowInteractorAxial = self.renderWindowAxial.GetInteractor()
        self.renderWindowInteractorCoronal = self.renderWindowCoronal.GetInteractor()
        self.renderWindowInteractorSagittal = self.renderWindowSagittal.GetInteractor()

        self.sphereWidgetAxial.SetInteractor(self.renderWindowInteractorAxial)
        self.sphereWidgetInteractionRotateGreenLineAxial.SetInteractor(
            self.renderWindowInteractorAxial
        )
        ...
        self.sphereWidgetInteractionRotateGreenLineAxial.SetCenter(
            center[0], (yMax + center[1]) / 2, center[2]
        )
        ...
        self.resliceAxial.SetInputConnection(self.reader.GetOutputPort())
        ...
        self.resliceCoronal.SetInputConnection(self.reader.GetOutputPort())
        ...
        self.resliceSagittal.SetInputConnection(self.reader.GetOutputPort())
        ...
        # Display
        self.actorAxial.GetMapper().SetInputConnection(
            self.resliceAxial.GetOutputPort()
        )
        self.actorCoronal.GetMapper().SetInputConnection(
            self.resliceCoronal.GetOutputPort()
        )
        self.actorSagittal.GetMapper().SetInputConnection(
            self.resliceSagittal.GetOutputPort()
        )
        ...
        def setupCameraAxialView(newPosition: Union[List, Tuple]) -> None:
            ...
            reverseProjectionVector = [
                cameraPosition[i] - focalPoint[i] for i in range(3)
            ]
            normReverseProjectionVector = vtkmath.Norm(reverseProjectionVector)
            vector = [newPosition[i] - focalPoint[i] for i in range(3)]
            translationInterval = [
                (
                    vtkmath.Dot(reverseProjectionVector, vector)
                    / math.pow(normReverseProjectionVector, 2)
                )
                * reverseProjectionVector[i]
                for i in range(3)
            ]
            self.rendererAxial.GetActiveCamera().SetPosition(
                [cameraPosition[i] + translationInterval[i] for i in range(3)]
            )
            self.rendererAxial.GetActiveCamera().SetFocalPoint(
                [focalPoint[i] + translationInterval[i] for i in range(3)]
            )
        ...
        def setupCameraCoronalView(newPosition: Union[List, Tuple]) -> None:
            ...
            self.rendererCoronal.GetActiveCamera().SetPosition(
                [cameraPosition[i] + translationInterval[i] for i in range(3)]
            )
            self.rendererCoronal.GetActiveCamera().SetFocalPoint(
                [focalPoint[i] + translationInterval[i] for i in range(3)]
            )
        ...
        def setupCameraSagittalView(newPosition: Union[List, Tuple]) -> None:
            ...
            self.rendererSagittal.GetActiveCamera().SetPosition(
                [cameraPosition[i] + translationInterval[i] for i in range(3)]
            )
            self.rendererSagittal.GetActiveCamera().SetFocalPoint(
                [focalPoint[i] + translationInterval[i] for i in range(3)]
            )
        ...
        def interactionEventHandleTranslateLinesAxialView(obj, event) -> None:
            ...
            translationInterval = [
                newPosition[i] - currentSphereWidgetCenter["axial"][i] for i in range(3)
            ]
            self.sphereWidgetInteractionRotateGreenLineAxial.SetCenter(
                [
                    currentSphereWidgetCenterRotateLinesAxial["green"][i]
                    + translationInterval[i]
                    for i in range(3)
                ]
            )
            currentSphereWidgetCenterRotateLinesAxial[
                "green"
            ] = self.sphereWidgetInteractionRotateGreenLineAxial.GetCenter()
        ...
        def interactionEventHandleTranslateLinesSagittalView(obj, event) -> None:
            ...
            # Set rotation position on green line in axial view
            translationInterval = [
                newPosition[i] - currentSphereWidgetCenter["sagittal"][i]
                for i in range(3)
            ]
            self.sphereWidgetInteractionRotateGreenLineAxial.SetCenter(
                [
        ...
        def interactionEventHandleRotateGreenLineAxialView(obj, event) -> None:
            ...
            v1 = [
                currentSphereWidgetCenterRotateLinesAxial["green"][i]
                - currentSphereWidgetCenter["axial"][i]
                for i in range(3)
            ]
            v2 = [
                newPosition[i] - currentSphereWidgetCenter["axial"][i] for i in range(3)
            ]
            angle = vtkmath.DegreesFromRadians(
                vtkmath.SignedAngleBetweenVectors(v1, v2, [0, 0, -1])
            )
            ...
            vtk.vtkMatrix4x4.Multiply4x4(
                self.resliceSagittal.GetResliceAxes(),
                self.rotationMatrix,
                self.resultMatrix,
            )
            ...
            for i in range(4):
                for j in range(4):
                    self.resliceSagittal.GetResliceAxes().SetElement(
                        i, j, self.resultMatrix.GetElement(i, j)
                    )
            ...
            vtk.vtkMatrix4x4.Multiply4x4(
                self.resliceCoronal.GetResliceAxes(),
                self.rotationMatrix,
                self.resultMatrix,
            )
            # Extract image after rotation
            for i in range(4):
                for j in range(4):
                    self.resliceCoronal.GetResliceAxes().SetElement(
                        i, j, self.resultMatrix.GetElement(i, j)
                    )
        ...
        # self.sphereWidgetAxial.RemoveAllObservers()
        self.sphereWidgetInteractionRotateGreenLineAxial.RemoveAllObservers()
        # self.sphereWidgetCoronal.RemoveAllObservers()
        # self.sphereWidgetSagittal.RemoveAllObservers()

        self.sphereWidgetAxial.AddObserver(
            vtkCommand.InteractionEvent, interactionEventHandleTranslateLinesAxialView
        )
        self.sphereWidgetInteractionRotateGreenLineAxial.AddObserver(
            vtkCommand.InteractionEvent, interactionEventHandleRotateGreenLineAxialView
        )

        self.sphereWidgetCoronal.AddObserver(
            vtkCommand.InteractionEvent, interactionEventHandleTranslateLinesCoronalView
        )

        self.sphereWidgetSagittal.AddObserver(
            vtkCommand.InteractionEvent,
            interactionEventHandleTranslateLinesSagittalView,
        )
        ...
        def mouseWheelEventHandleCoronalView(obj, event) -> None:
            ...
            self.sphereWidgetInteractionRotateGreenLineAxial.SetCenter(
                [
                    self.sphereWidgetInteractionRotateGreenLineAxial.GetCenter()[i]
                    + translationInterval[i]
                    for i in range(3)
                ]
            )
        ...
        self.interactorStyleAxial.AddObserver(
            vtkCommand.MouseWheelForwardEvent, mouseWheelEventHandleAxialView
        )
        self.interactorStyleAxial.AddObserver(
            vtkCommand.MouseWheelBackwardEvent, mouseWheelEventHandleAxialView
        )

        self.interactorStyleCoronal.AddObserver(
            vtkCommand.MouseWheelForwardEvent, mouseWheelEventHandleCoronalView
        )
        self.interactorStyleCoronal.AddObserver(
            vtkCommand.MouseWheelBackwardEvent, mouseWheelEventHandleCoronalView
        )

        self.interactorStyleSagittal.AddObserver(
            vtkCommand.MouseWheelForwardEvent, mouseWheelEventHandleSagittalView
        )
        self.interactorStyleSagittal.AddObserver(
            vtkCommand.MouseWheelBackwardEvent, mouseWheelEventHandleSagittalView
        )
